...
class SwitcherEntity(CoordinatorEntity[SwitcherDataUpdateCoordinator]):
    ...
    async def _async_call_api(self, api: str, *args: Any, **kwargs: Any) -> None:
        ...
        if error or not response or not response.successful:
            # Surface the failure to the caller, but do not force the device
            # unavailable on a single failed command. Availability is driven by
            # the broadcast stream and, when that stops, by the coordinator poll,
            # so a transient command error no longer flaps the entity.
